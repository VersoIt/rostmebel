...
<template>
  <div class="min-h-screen flex flex-col font-sans selection:bg-brand-gold selection:text-white">
      ...
      <transition name="menu">
        <div v-if="isMenuOpen" class="fixed inset-0 z-[200] bg-brand-brown flex flex-col p-10">
          <div class="flex justify-between items-center mb-20">
            ...
            <button @click="isMenuOpen = false" class="w-12 h-12 bg-white/5 rounded-full flex items-center justify-center text-white">
              <LucideX :size="32" />
            </button>
          </div>
          ...
          <div class="mt-auto pt-10 border-t border-white/10">
            <p class="text-white/40 uppercase tracking-widest text-xs mb-4 font-bold">Связаться с нами</p>
            <a href="tel:+79787631603" class="text-2xl text-white font-bold tracking-tight">+7 (978) 763-16-03</a>
          </div>
        </div>
      </transition>
    </Teleport>
    ...
    <!-- Footer -->
    <footer class="bg-brand-cream border-t border-brand-brown/5 pt-32 pb-16 px-6">
      <div class="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-16">
        <div class="col-span-1 md:col-span-2">
          <div class="flex items-center gap-3 mb-8">
            <div class="w-10 h-10 bg-brand-brown text-brand-gold rounded-xl flex items-center justify-center font-serif text-xl font-bold">Р</div>
        ...
        <div>
          <h4 class="font-bold text-xs uppercase tracking-[0.2em] text-brand-gold mb-8">Навигация</h4>
          <ul class="space-y-4">
            <li><router-link to="/catalog" class="font-bold text-brand-brown/80 hover:text-brand-gold transition-colors uppercase text-[10px] tracking-widest">Все проекты</router-link></li>
            <li><router-link to="/contact" class="font-bold text-brand-brown/80 hover:text-brand-gold transition-colors uppercase text-[10px] tracking-widest">Контакты</router-link></li>
            <li><router-link to="/favorites" class="font-bold text-brand-brown/80 hover:text-brand-gold transition-colors uppercase text-[10px] tracking-widest">Избранное</router-link></li>
          </ul>
        </div>

        <div>
          <h4 class="font-bold text-xs uppercase tracking-[0.2em] text-brand-gold mb-8">Контакты</h4>
      ...
      <div class="max-w-7xl mx-auto mt-32 pt-8 border-t border-brand-brown/5 flex flex-col md:flex-row justify-between items-center gap-4">
        <p class="text-brand-brown/30 text-xs font-bold uppercase tracking-widest">© 2024 РОСТ Мебель. Все права защищены.</p>
        <div class="flex gap-8">
          <a href="#" class="text-brand-brown/30 text-[10px] uppercase tracking-widest hover:text-brand-gold transition-colors font-bold">Политика конфиденциальности</a>
        </div>
      </div>
    </footer>
  </div>
...
.selection\:bg-brand-gold ::selection { background-color: #C9A84C; color: white; }
</style>
